 function [new_nodes, new_fibers] = clip_net(nodes, fibers, clipbox)
    global prog_box num_nets N
...
if min_length ~= 0
    ...
    message = sprintf('Creating network %i of %i. . . Clip net good, no zero length fibs', N, num_nets);
    ...
    message = sprintf('Creating network %i of %i. . . Clip net bad, zero length fibs', N, num_nets);
    prog_box.Message = message;
end
...
if length( all_group_fib_num ) ~= length( unique( all_group_fib_num ) )
  ...
  disp('clip_net: good -- fiber categories mutually exclusive...');
    message = sprintf('Creating network %i of %i. . . Clip net fibs mutually exclusive', N, num_nets);
    prog_box.Message = message;
end
...
function [node_inside] = check_inside_not_on_boundary(x, y, z, clip_bounds)
